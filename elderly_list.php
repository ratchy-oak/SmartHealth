...
if (!$project_year || !$hospital_id) {
    header("Location: elderly_form.php?error=invalid_params");
    exit();
}
...
$page_title = "รายชื่อผู้สูงอายุ/ผู้พิการ ปี " . htmlspecialchars($project_year);
...
$stmt = $conn->prepare("
    ...
    WHERE hospital_id = ? 
      AND project_year = ?
      AND status IN ('ผู้สูงอายุ', 'ผู้พิการ')
...
<body class="page-patient-care-list"> <nav class="navbar navbar-expand-lg navbar-light bg-white border-bottom shadow-sm py-3 sticky-top">
      <div class="container d-flex align-items-center justify-content-between">
          <a href="elderly_form.php" class="d-flex align-items-center text-dark text-decoration-none" aria-label="ย้อนกลับ"><i class="bi bi-arrow-left fs-2" aria-hidden="true"></i></a>
          <div class="text-center position-absolute top-50 start-50 translate-middle"><span class="fw-bold fs-5 text-truncate px-2"><?= $page_title ?></span></div>
    ...
    <div class="container my-5">
        <?php if (empty($patients)): ?>
            <div class="alert alert-info text-center">ไม่พบรายชื่อผู้สูงอายุหรือผู้พิการสำหรับปีโครงการนี้</div>
        <?php else: ?>
            <div class="row g-4">
                    ...
                    <div class="col-12 col-md-6 col-lg-4">
                        <div class="care-list-card">
                            ...
                            <div class="d-grid gap-2 d-sm-flex">
                                <a href="elderly_history.php?patient_id=<?= $patient['id'] ?>" class="btn btn-outline-dark flex-sm-fill">
                                    <i class="bi bi-clock-history me-1"></i>ดูประวัติ
                                </a>
                                <a href="elderly_assessment_form.php?id=<?= $patient['id'] ?>" class="btn btn-care flex-sm-fill">
                                    <i class="bi bi-pencil-square me-1"></i>ประเมิณ
                                </a>
                            </div>
                        </div>
                    </div>
